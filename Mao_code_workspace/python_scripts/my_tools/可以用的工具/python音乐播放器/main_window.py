...
from PyQt5.QtWidgets import QApplication, QFileDialog, QInputDialog, QListWidgetItem, QWidget

from NCOperation import NeteaseMusic
from commonl import *
from music_window_UI import Ui_my_music

wyy_music = NeteaseMusic()
# 首页歌单标题按规定长度显示
...
class Main_window(QWidget, Ui_my_music):
    ...
    def initUI(self):
        ...
        self.time_line.sliderMoved.connect(
            self.music_time_adjust)  # 拖动进度条改变播放进度
        self.time_line.sliderReleased.connect(
            self.music_time_adjust_over)  # 拖动进度条调整完成，拖动调节分成两步，播放本地音乐时可去除杂音
        self.listview.itemDoubleClicked.connect(
            self.music_double_click_thread)  # 双击播放音乐
        self.src_choose.currentTextChanged.connect(
            self.first_page_thread)  # 更改音乐源时触发的事件
        ...
        self.new_playlist.clicked.connect(
            self.create_playlist_thread)  # 新建我的歌单线程
        self.delete_playlist.clicked.connect(self.delete_my_playlist)  # 删除我的歌单
        self.add_to_playlist.clicked.connect(
            self.add_my_playlist_thread)  # 将当前播放音乐加入我的歌单线程
        self.myplaylist_list.itemDoubleClicked.connect(
            self.click_my_playlist_thread)
    ...
    def downloads_music_thread(self):
        if self.disc_title.text():
            self.date.folder_path = QFileDialog.getExistingDirectory(
                self, "下载保存路径", self.date.folder_path)
    ...
    def local_music_thread(self):
        self.date.folder_path = QFileDialog.getExistingDirectory(
            self, "选取音乐文件夹", self.date.folder_path)
    ...
    def create_playlist_thread(self):
        ...
        create = threading.Thread(
            target=self.create_my_playlist, args=(text, ok))
        create.start()
    ...
    def add_my_playlist_thread(self):
        if self.disc_title.text():
            text, ok = QInputDialog.getItem(
                self, "选择要加入的歌单", "歌单名", self.date.my_playlist_name, 0, False)
            add_playlist = threading.Thread(
                target=self.add_my_playlist, args=(text, ok))
            add_playlist.start()
    ...
    def timeout_process(self):
        if self.music_start_stop.objectName() == "stop":
            ...
            if self.time_pre.text() == "00:02":
                ...
                self.time_line.setRange(
                    0, int(time_long / 1000) + 1)  # 设置进度条范围
                self.time_next.setText(  # 设置音频长度的显示
                    str(int(time_long / 1000) // 60).zfill(2) + \
                    ':' + str(int(time_long / 1000) % 60).zfill(2)
                )
    ...
    def set_first_date(self):
        if self.src_choose.currentText() == "网易云":
            ...
            self.listview.clear()
            self.date.img_url, self.date.bt_name, self.date.bt_id = wyy_music.wyy_first_page()
            for i in range(8):
                img = QPixmap()
                img.loadFromData(get_requests(self.date.img_url[i], 'c'))
                self.img_list[i].setPixmap(img)
                self.title_list[i].setText(
                    first_title_display(self.date.bt_name[i]))
    ...
    def first_playlist_dj(self):
        ...
        if self.src_choose.currentText() == "网易云":
            if id_str[1] == 'p':
                self.date.music_name, self.date.music_id = wyy_music.playlist_info(
                    id_str.split('=')[1])
                ...
                self.player.setMedia(QMediaContent(
                    QUrl(wyy_music.dj_url(id_str.split('=')[1]))))  # 设置音频网址
    ...
    def list_view_song(self):
        ...
        if self.listview_lable.objectName() == "local":
            ...
            if self.src_choose.currentText() == "网易云":
                url = wyy_music.song_url(music_id)
                if url == "":
    ...
    def show_search(self):
        self.lock.acquire()
        if self.src_choose.currentText() == "网易云":
            self.date.music_name, self.date.music_id = wyy_music.single_search(
    ...
    def write_music_bin(self):
        with open(self.date.folder_path + '/' + self.disc_title.text() + '.mp3', 'wb') as f:
            f.write(get_requests(self.disc.objectName(), 'c'))
    ...
    def delete_my_playlist(self):
        text, ok = QInputDialog.getItem(
            self, "选择要删除的歌单", "歌单名", self.date.my_playlist_name, 0, False)
        if ok:
    ...
    def load_playlist(self):
        ...
        for i in os.listdir(os.path.dirname(__file__)+"./my_playlist"):
            name = i.split('.')[0]
            ...
            with open("./my_playlist/" + i, 'r', encoding='utf-8') as f:
                self.date.my_playlist_text[name] = json.loads(
                    f.readline())  # 将其中的数据加载进来
        self.lock.release()
        ...
        if ok:
            if self.disc.objectName() == "":
                ...
                if data in self.date.my_playlist_text[text]["playlist"]:
                    ...
                    with open("./my_playlist/" + text + ".json", 'w', encoding='utf-8') as f:
                        json.dump(
                            self.date.my_playlist_text[text], f, ensure_ascii=False)

...
class NeteaseMusic:
    ...
    def down_img(self, img_url_list, path='./homerecommendationico'):
        """  下载首页推荐歌单封面照片"""
        if not os.path.exists(path):  # 如果路径不存在就创建
            os.mkdir(path)
        for j in img_url_list:
            ico_name = str(time())
            with open(f"{path}/{ico_name}.jpeg", "wb") as f:
                f.write(get_requests(j, 'c'))  # 写入数据
        print("下载完毕，保存路径：" + path)

    def wyy_first_page(self):
        """ 获取首页歌单,返回图片url、名字title、id号href"""
        # 先对首页进行简单处理
        doc = pq(get_requests("https://music.163.com/discover"))
    ...
    def playlist_info(self, playlist_id):
        ...
        doc = pq(get_requests(url="https://music.163.com/playlist?id=" + playlist_id))
    ...
    def dj_url(self, dj_id):
        """ 传入dj电台的id号，获取dj电台的音频文件"""
        url = f"https://api.imjad.cn/cloudmusic/?type=dj&id={dj_id}"
        response = get_requests(url, 'j')
        return response['data'][0]['url']

    def song_url(self, song_id, br=320000):
        """ 通过指定的歌曲song_id获取歌曲的音频二进制文件"""
        # 通过urls获取歌曲下载链接
        url1 = f"https://api.imjad.cn/cloudmusic/?type=song&id={song_id}&br={br}"
        url = get_requests(url1, 'j')['data'][0]['url']
        return url

    def single_search(self, name):
        """ 单曲搜索"""
        ...
        songs = get_requests(url, 'j')['result']['songs']
        for i in songs:

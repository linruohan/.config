...
from PyQt5.QtWidgets import QDesktopWidget,QHBoxLayout, QGridLayout, QLabel, QPushButton, QVBoxLayout, QWidget
...
class UiBase(QWidget):
    ...
    ''')
        ...
        self.init_title_label()  # 安放标题栏标签
        self.setWindowTitle = self._set_title_text(
            self.setWindowTitle)  # 用装饰器将设置WindowTitle名字函数共享到标题栏标签上
        self.setWindowTitle("工作日报")
        self._CloseButton = QTitleButton(b'\xef\x81\xb2'.decode("utf-8"),
                                         self)
        self.set_close_button(True)
        self._MinimumButton = QTitleButton()
        self.set_min_max_buttons(True)
        self.main_layout = QGridLayout()
        self.init_layout()  # 设置框架布局
        self.center()
    ...
    def set_close_button(self, bool):
    ...
    def set_min_max_buttons(self, bool):
        ...
        if bool:
            ...
            self._MaximumButton.clicked.connect(
                self._change_normal_button)  # 按钮信号连接切换到恢复窗口大小按钮函数

    def _change_normal_button(self):
        # 切换到恢复窗口大小按钮
        try:
            ...
            self._MaximumButton.clicked.connect(
                self._change_max_button)  # 重新连接信号和槽
        except Exception as e:
            print(e)

    def _change_max_button(self):
        # 切换到最大化按钮
        try:
            self.showNormal()
            ...
            self._MaximumButton.clicked.connect(self._change_normal_button)
        except Exception as e:
            print(e)
    ...
    def mousePressEvent(self, event):
        ...
        if (event.button() == Qt.LeftButton) and (
                event.pos() in self._corner_rect):
            # 鼠标左键点击右下角边界区域
            self._corner_drag = True
            event.accept()
        elif (event.button() == Qt.LeftButton) and (
                event.pos() in self._right_rect):
            # 鼠标左键点击右侧边界区域
            self._right_drag = True
            event.accept()
        elif (event.button() == Qt.LeftButton) and (
                event.pos() in self._bottom_rect):
            # 鼠标左键点击下侧边界区域

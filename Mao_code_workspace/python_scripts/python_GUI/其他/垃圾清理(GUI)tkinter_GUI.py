...
import os.path
import threading
rubbishEXT = ['.tmp', '.bak', '.old', '.wbk', '.xlk',
              '._mp',  '.gid', '.chk', '.syd', '.$$$', '.@@@', '.~*']
...
class Window:
    def __init__(self):
        ...
        # 创建标签,用于显示状态信息
        self.progress = tkinter.Label(
            self.root, anchor=tkinter.W, text="状态", bitmap='hourglass', compound='left')
    ...
    def MenuAbout(self):
        tkinter.messagebox.showinfo('Soto_Cleaner', '本软件版权归:王小涛_同學 所有,欢迎使用!')
    ...
    def MenuScanRubbish(self):
        result = tkinter.messagebox.askquestion(
            'Soto_Cleaner', '扫描垃圾文件需较长时间,是否继续?')
        if result == 'no':
            return
        tkinter.messagebox.showinfo('Soto_Cleaner', '马上开始扫描垃圾文件!')
        # self.ScanRubbish()
        self.drives = GetDrives()
        t = threading.Thread(target=self.ScanRubbish,
                             args=(self.drives,))  # 创建线程
        t.start()  # 开始线程

    def MenuDelRubbish(self):
        result = tkinter.messagebox.askquestion(
            'Soto_Cleaner', '删除垃圾文件时间较长,是否继续?')
        if result == 'no':
            return
        tkinter.messagebox.showinfo('Soto_Cleaner', '马上开始删除垃圾文件!')
        self.drives = GetDrives()
        t = threading.Thread(target=self.DeleteRubbish, args=(self.drives,))
        t.start()

    def MenuScanBigFile(self):
        s = tkinter.simpledialog.askinteger('Soto_Cleaner', '请设置文件大小(M)')
        t = threading.Thread(target=self.ScanBigFile, args=(s,))
        t.start()

    def MenuSearchFile(self):
        s = tkinter.simpledialog.askstring('Soto_Cleaner', '请输入文件名的部分字符')
        ...
        for drive in scanpath:
            for root, dirs, files in os.walk(drive):
                try:
                    ...
                    print(e)
                    pass
        self.progress['text'] = '找到 %s 个垃圾文件,共占用 %.2f M磁盘空间' % (
        ...
        for drive in scanpath:
            for root, dirs, files in os.walk(drive):
                try:
                    ...
                    print(e)
                    pass
        self.progress['text'] = '删除 %s 个垃圾文件,收回 %.2f M磁盘空间' % (
            total, filesize/1024/1024)
            ...
            for root, dirs, files in os.walk(drive):
                for file in files:
                    try:
                        ...
                        pass
        self.progress['text'] = '找到%s个超过 %s M的大文件' % (
            total, filesize/1024/1024)

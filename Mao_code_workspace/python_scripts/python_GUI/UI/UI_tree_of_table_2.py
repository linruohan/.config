...
import tree_of_table


class ServerModel(tree_of_table.TreeOfTableModel):
    ...
    def data(self, index, role):
        if role == Qt.DecorationRole:
            ...
            if node is None:
                return QVariant()
            if isinstance(node, tree_of_table.BranchNode):
                if index.column() != 0:
                    return QVariant()
        ...
        return tree_of_table.TreeOfTableModel.data(self, index, role)

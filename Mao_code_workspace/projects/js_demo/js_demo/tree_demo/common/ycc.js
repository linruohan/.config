    ...
    Ycc.Layer.prototype.renderAllToCtx = function(ctx) {
        ...
        if (this.useCache && rect && this.renderCacheRect) {
            ...
            var x = rect.x < 0 ? 0 : rect.x,
                y = rect.y < 0 ? 0 : rect.y,
                width = rect.width > stageW ? stageW : rect.width,
                height = rect.height > stageH ? stageH : rect.height;
            ctx.save();
            ctx.beginPath();
            ctx.strokeStyle = 'red';
            ctx.strokeWidth = 2;
...
(function(Ycc) {
    ...
    Ycc.UI.BrokenLine.prototype.computeUIProps = function() {
        ...
        var minx = this.pointList[0].x,
            miny = this.pointList[0].y,
            maxx = this.pointList[0].x,
            maxy = this.pointList[0].y;
